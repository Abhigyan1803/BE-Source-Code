...
import com.example.demo.repository.LocationStateOfOfficesRepo;
...
@Service
public class LocationStateOfOfficesServiceImpl implements LocationStateOfOfficesService {
	...
	@Autowired
	LocationStateOfOfficesRepo officesRepo;

	@Override
	public Map<Object, Object> addDetails(MultipartFile doc, LocationStateOfOffices record,
		...
		try {
			...
			LocationStateOfOffices saved = officesRepo.save(record);
			if (saved != null) {
				FileWritting.createLog((HttpServletRequest) servletRequest, saved.getId() + ",added,"
						+ "location_state," + ConstantMessage.RECORD_ADDED_SUCCESSFULLY + "," + new Date());
	...
	@Override
	public Map<Object, Object> updateDetails(MultipartFile doc, LocationStateOfOffices update,
			ServletRequest servletRequest) {
		HashMap<Object, Object> map = new HashMap<>();
		try {
			LocationStateOfOffices existing = officesRepo.findById(update.getId()).get();
			if (existing != null) {
				if (doc != null && !doc.isEmpty()) {
					String filename = FileUploader.uploadProfileImage(doc, UploadDir);
				...
				LocationStateOfOffices updated = officesRepo.save(existing);
				if (updated != null) {
					FileWritting.createLog((HttpServletRequest) servletRequest, updated.getId() + ",update,"
							+ "location_state," + ConstantMessage.RECORD_UPDATED_SUCCESSFULLY + "," + new Date());
	...
	public Map<Object, Object> viewById(Long id) {
		HashMap<Object, Object> map = new HashMap<>();
		try {
			LocationStateOfOffices record = officesRepo.findById(id).get();
			if (record != null) {
	...
	public Map<Object, Object> activeDeactiveStatus(Long id, int status, ServletRequest servletRequest) {
		...
		try {
			...
			if (record != null) {
				...
				officesRepo.save(record);

				FileWritting.createLog((HttpServletRequest) servletRequest, record.getId() + ",update-status,"
						+ "location_state," + ConstantMessage.STATUS_UPDATED_SUCCESSFULLY + "," + new Date());
	...
	public Map<Object, Object> getAllDetails() {
		...
		try {
			...
			List<LocationStateOfOffices> list = officesRepo.findAllByStatusNotInOrderByIdDesc(deletedStatus);
			if (!list.isEmpty()) {
